	...
	while (!f.eof()) {
		...
		if (parameters.size() == 100) {
			...
			if (parameters[0] == "v") {
				...
				cout << parameters.size() << endl;
				vector<GLint64>vIndexSets;
				vector<GLint64>vIndexSets1;
				...
				for (int i = 1; i < 4; i++) {
					...
					GLint64 index = atof(ans.c_str());
					cout << index << "$" << endl;
					if (index < 0)
					{
						index = index + vSets.size();
					}
					else
					{
						index = index--;
				...
				if (parameters.size() == 5)
				{
					...
					for (int i = 3; i < 5;)
					{
						...
						GLint64 index1 = atof(ans.c_str());
						if (index1 < 0)
						{
							index1 = index1 + vSets.size();
						}
						else
						{
							index1 = index1--;
